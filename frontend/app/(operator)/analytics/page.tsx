  ...
  return (
    <div>
      ...
      <p className="muted" style={{ fontSize: 12, marginTop: 10 }}>
        {outcomes.analyzed_count} analyzed · coverage {pct(outcomes.coverage_pct)} of connected
      </p>
    </div>
  );
}
...
function TimeseriesChart({ points }: { points: TimeseriesPoint[] }) {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <svg
        viewBox={`0 0 ${Math.max(560, points.length * (barW + 4))} 140`}
        style={{ width: "100%", height: 140, display: "block" }}
        aria-label="Call volume timeseries"
      >
        {points.map((p, i) => {
          const totalH = (p.total / maxTotal) * 100;
          const connH = (p.connected / maxTotal) * 100;
          const x = i * (barW + 4);
          return (
            <g key={p.date}>
              {/* total bar (background) */}
              <rect x={x} y={120 - totalH} width={barW} height={totalH}
                fill="rgba(79,140,255,0.18)" rx={3} />
              {/* connected bar (foreground) */}
              <rect x={x} y={120 - connH} width={barW} height={connH}
                fill="rgba(79,140,255,0.70)" rx={3} />
              {/* date label every N bars */}
              {(i === 0 || i === points.length - 1 || (points.length > 6 && i % Math.ceil(points.length / 6) === 0)) && (
                <text x={x + barW / 2} y={136} textAnchor="middle" fontSize={9} fill="var(--muted)">
                  {p.date.slice(5)}
                </text>
              )}
            </g>
        ...
        {/* legend */}
        <rect x={0} y={0} width={10} height={8} fill="rgba(79,140,255,0.70)" rx={2} />
        <text x={14} y={8} fontSize={9} fill="var(--muted)">Connected</text>
        <rect x={70} y={0} width={10} height={8} fill="rgba(79,140,255,0.18)" rx={2} />
        <text x={84} y={8} fontSize={9} fill="var(--muted)">Total</text>
      </svg>
    </div>
  );
}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.agent_id}>
              ...
              <td>{r.call_volume.connected}</td>
              <td>{pct(r.call_volume.connection_rate)}</td>
              <td className="muted">{dur(r.duration.avg_seconds)}</td>
              <td>{r.outcomes.analyzed_count > 0 ? pct(r.outcomes.booking.pct_of_analyzed) : "—"}</td>
            ...
            <div className="card">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, fontSize: 12 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ width: 12, height: 12, background: "rgba(79,140,255,0.70)", borderRadius: 3, display: "inline-block" }} />
                  Connected
                </span>
                <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ width: 12, height: 12, background: "rgba(79,140,255,0.18)", borderRadius: 3, display: "inline-block" }} />
                  Total
                </span>
              </div>
