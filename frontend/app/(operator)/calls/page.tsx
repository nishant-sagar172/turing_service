...
function CallDetailDrawer({ clientId, callId, onClose }: { clientId: string; callId: string; onClose: () => void }) {
  ...
  async function runAnalyze() {
    setAnalyzing(true);
    try {
      const result = await adminApi.analyzeClientCall(clientId, callId);
      setAnalyzeResult(result);
      const updated = await adminApi.getClientCallDetail(clientId, callId);
      setDetail(updated);
    } catch (e: unknown) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "var(--panel)", borderLeft: "1px solid var(--glass-border)",
        ...
        {loading ? (
          ...
          <>
            <table style={{ marginBottom: 16 }}>
              <tbody>
                <tr><td className="muted" style={{ width: 130, paddingRight: 12 }}>Status</td><td><StatusBadge status={detail.status} /></td></tr>
                ...
                {detail.recording_url && (
                  <tr>
                    ...
                    <td><a href={detail.recording_url} target="_blank" rel="noreferrer" style={{ color: "var(--accent)", fontSize: 13 }}>▶ Play recording</a></td>
                  </tr>
                )}
              </tbody>
            </table>
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    ...
                    {detail.analysis.model_used && (
                      <span className="muted" style={{ fontSize: 11 }}>{detail.analysis.model_used}</span>
                    )}
                  </div>
    ...
    <div className="card">
      ...
      <div className="row" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end" }}>
          <button className="secondary" onClick={applyFilters}>Filter</button>
        </div>
      </div>
      ...
      {selectedId && <CallDetailDrawer clientId={clientId} callId={selectedId} onClose={() => setSelectedId(null)} />}
    </div>
  );
}
