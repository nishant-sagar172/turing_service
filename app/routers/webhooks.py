...
from app.services.store import extract_voice_batch_id, upsert_call_from_execution
...
async def _run_analysis(call_id: str, settings: Settings) -> None:
    """Background task: opens its own session, fetches call + config, runs LLM."""
...
async def voice_webhook(
    ...
    # Fire-and-forget analysis for completed calls — existing logic above is untouched.
    if call.status == "completed" and call.transcript:
