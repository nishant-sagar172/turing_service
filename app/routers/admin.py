...
from app.schemas.agents import AgentVariables
...
async def get_drift(
    ...
    await _get_client_or_404(session, client_id)
    return await agent_sync.list_drift_events(session, client_id)
...
async def admin_get_call(
    ...
    if call.analysis is None:
        res = await session.execute(select(CallAnalysis).where(CallAnalysis.call_id == call.id))
        call.analysis = res.scalar_one_or_none()
