...
async def get_batch_executions(
    ...
    await _get_batch_or_404(session, tenant, batch_id)
    result = await voice_engine.get_batch_executions(batch_id)

    responses: list[ExecutionResponse] = []
    for item in result if isinstance(result, list) else []:
        if isinstance(item, dict):
            item.setdefault("batch_id", batch_id)
            await upsert_call_from_execution(session, item, client_id=tenant.client_id)
